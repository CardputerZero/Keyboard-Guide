...
private:
    ...
    std::array<std::unique_ptr<smooth_ui_toolkit::lvgl_cpp::Image>, 2> _fn_sequence_images;
    ...
    uint32_t _shown_result_revision    = 0;

    void render(const GuideLessonState& state);
    void applyTransforms(uint32_t now_ms);
    void playResultPop(lv_obj_t* object, int x, int y, int width, int height);
