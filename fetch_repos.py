...
def clone_or_update_repo(
    ...
    if with_submodules:
        subprocess.run(
            ["git", "-C", path, "submodule", "update", "--init", "--recursive"], check=True
        )

    if patch_path:
        patch_full_path = (
            patch_path if os.path.isabs(patch_path) else str(Path.cwd() / patch_path)
        )
        check_result = subprocess.run(["git", "-C", path, "apply", "--check", patch_full_path])
...
def load_repos():
    ...
    with open(script_dir / "repos.json") as file:
...
def ensure_dependencies():
    ...
    print(f"Keyboard Guide: fetching missing dependencies: {names}")
